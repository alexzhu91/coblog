...
gl base lnsize bm_month TotInsHoldper ret lret_6  coverage , abs(id) cluster(mdate) 
...
gl base noq lnsize bm_month TotInsHoldper ret lret_6  coverage , abs(id) //cluster(mdate) 
foreach a of varlist $beta {
...
*---------------shortsell pilot as IV of corank/co_p, strong IV, 2nd stage no result-----------------------------------
 ...
 reghdfe beta_led beta turnover noq lnsize bm_month TotInsHoldper ret lret_6 coverage soe olym (co_p_r=soe_olym), abs(id ym) cluster(mdate) //used to work, but not now!!!!qui eststo:reghdfe `y'_led `y' noq $control soe (`a'=l.soe_olym1) , abs(id ym) cluster(mdate) 

  ******************only olym as IV
  reghdfe beta_led beta turnover lnsize bm_month TotInsHoldper ret lret_6 coverage summer (co_p_r=olympic), abs(id year) cluster(mdate) //all insignif
  reghdfe beta_led beta noq turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (co_p_r=olympic), abs(year)  ffirst stage(first) //can't add noq, since it strongly weaken cop,but result is there
