/* this is weekly data, no xueqiu data yet
2016/7/26
ta noq year exchg
...
cd .\log
cap log close
log using 0726.txt,t append

cd ..\input
use weekly,clear

*sum amihud
duplicates rep stkcd week,g(dup)
encode stkcd, gen(id)
gen weekly = wofd(week)
format weekly %tw
replace year=year(week)
*br year* qtr* date weekly
*use 09 stock to exclude newly listed effect
g bef_10=(year<2010)
bys  id: egen _tmp1=sum(bef_10)
g _tmp2=substr(stkcd,1,1) 
keep if  _tmp2 !="2" & _tmp2 !="9"  //_tmp1>1  &
drop _tmp1 _tmp2 
*keep if year>2009
...
bys weekly: cumul(co_p), gen(co_rank) equal
//bys weekly: cumul(co_xq), gen(co_rank_x) equal
bys weekly: cumul(single_p), gen(single_rank) equal
bys weekly: cumul(num_cmt_p5w), gen(cmt_rank) equal
...
*bys id (weekly): g syn_lead=f.syn
...
g olympic=(week<mdy(8,13,2012)&week>mdy(7,27,2012))
g fifa=(week<mdy(7,13,2014)&week>mdy(6,12,2014))
g month=month(week)
...
*/
  gl time summer rep1 rep4 jan aug

gl control   turnover lnsize bm_month TotInsHoldper ret lret_6  coverage 
gl base lnsize bm_month TotInsHoldper ret lret_6  coverage , abs(id) cluster(weekly) 
gl full noq dist_stk_p $control 
gl beta beta syn amihud pricedelay tot_vol idrisk_std turnover  skew ivol_unr ivol_res
global less $control , abs(id week) cluster(weekly) 
global fe $full , abs(id week) cluster(weekly)  //not cluster id is same
...
xtfmb ret_led co_rank $control if !noq, lag(1)
xtfmb ret_led   syn amihud pricedelay  $control, lag(3) //failed, since weekly is not regular
...
reghdfe  idrisk_std_led ans_rate ans_ask lndelay $less //negat, delay is skewed, so shld ln
 ...
 reghdfe beta_led beta co_p_r $full, abs(id week) cluster(weekly) //drop beta won't help
*---------------Which measure is sig after clustering, stambough no result-----------------------------------
preserve
*keep  if weekly>600 //&  coblog ==1
outreg,clear
eststo clear
...
eststo clear
gl time summer  jan aug //rep1 rep4
sum dist_stk_p
reghdfe dist_stk_p olym fifa $time  i.year , abs(id )  //can't cluster(mdate), since it'll kill everything in common during olym month lower indeed
*fifa is less significant than olym
reghdfe dist_stk_p olym fifa $time i.fy_qtr  i.year , abs(id ) //i.month can't, since collinear

 reghdfe f.total olym summer  fy_qtr i.year , abs(id ) cluster(weekly)  //lower indeed
 
gl cop lncmt_p  lnco  lnuser_p co_p_r  user_deseason  cmt_rank co_rank single_rank 
gl beta beta syn amihud pricedelay tot_vol idrisk_std turnover skew
gl base noq lnsize bm_month TotInsHoldper ret lret_6  coverage , abs(id) //cluster(weekly) 
foreach a of varlist $beta {
*qui eststo: reghdfe f.`a' olym soe soe_olym summer rep1 rep4 i.year $base 
*qui eststo: reghdfe f.`a' olym summer rep1 rep4 i.year $base 

qui eststo: reghdfe f.`a' `a' f.(olym $time i.year) $base 
}
eststo clear
gl base lnsize bm_month TotInsHoldper ret lret_6  coverage , abs(id) //cluster(weekly) 
foreach a of varlist $cop {
*qui eststo: reghdfe `a' olym  $time i.year $base 
qui eststo: reghdfe `a' olym soe soe_olym $time i.year $base 
qui eststo: reghdfe f.`a'  olym soe soe_olym $time i.year $base 
*qui eststo: reghdfe f.`a'  f.(olym $time i.year) $base 
}
...
probit noq olym soe soe_olym $time i.year $control
preserve
keep if !noq
reghdfe beta_led beta noq turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (co_p_r=olympic), abs(year)  ffirst stage(first) //can't add noq, since it strongly weaken cop,but result is there

reghdfe co_p_r syn olym soe soe_olym $time i.year $base 

*try to replicate the previous sign soe_olym result
reghdfe f.co_p_r  olym soe soe_olym summer rep1 rep4 jan i.year $base //yes 
reghdfe co_p_r  olym soe soe_olym summer rep1 rep4 jan i.year $base //no
reghdfe F.co_p_r  fifa soe soe_fifa summer rep1 rep4 jan i.year $base //YES, +FIFA
reghdfe co_p_r  fifa soe soe_fifa summer rep1 rep4 jan jun_july i.year $base //YES,-FIFA w/o jun_july; with jun_july, non-sign

*---------------Garch for vol-----------------------------------


*---------------shortsell pilot as IV of corank/co_p, strong IV, 2nd stage no result-----------------------------------
 reghdfe amihud_led amihud turnover lnsize bm_month TotInsHoldper ret lret_6 coverage  (co_p_r=ss_pilot), abs(id week) cluster(weekly)
 reghdfe beta_led beta turnover lnsize bm_month TotInsHoldper ret lret_6 coverage  (co_rank=l.ss_pilot), abs(id week) cluster(weekly)
  reghdfe beta_led beta turnover lnsize bm_month TotInsHoldper ret lret_6 coverage  (lnuser_p=ss_pilot), abs(id week) cluster(weekly)

 reghdfe beta_led beta turnover noq lnsize bm_month TotInsHoldper ret lret_6 coverage soe olym (co_p_r=soe_olym), abs(id week) cluster(weekly) //used to work, but not now!!!!qui eststo:reghdfe `y'_led `y' noq $control soe (`a'=l.soe_olym1) , abs(id week) cluster(weekly) 

  ******************only olym as IV
  reghdfe beta_led beta turnover lnsize bm_month TotInsHoldper ret lret_6 coverage summer (co_p_r=olympic), abs(id year) cluster(weekly) //all insignif
  reghdfe beta_led beta noq turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (co_p_r=olympic), abs(year)  ffirst stage(first) //can't add noq, since it strongly weaken cop,but result is there
  reghdfe beta_led beta turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (single_rank=olympic), abs(year)  ffirst //weak,not sign
  reghdfe beta_led beta turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (co_rank=olympic), abs(year)  ffirst stage(first) //weak, but negative
  est restore reghdfe_first1
  *can't cluster(year), otherwise wil not full rank

  
 reghdfe syn_led syn noq turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (co_p_r=olympic), abs(year)  ffirst stage(first) //negative; drop noq, become stronger
 reghdfe syn_led syn turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (user_deseason=olympic), abs(year)  ffirst stage(first) //negative; same as cop, user_p not working

 reghdfe pricedelay_led turnover lnsize bm_month TotInsHoldper ret lret_6 coverage $time (user_deseason=olympic), abs(year)  ffirst stage(first) //negative; same as cop, user_p not working
...
qui eststo:reghdfe `y'_led `y' noq $control soe `a' , abs(id week) cluster(weekly) ffirst  
qui eststo:reghdfe `y'_led `y' noq $control soe (`a'=l.ss_pilot) , abs(id week) cluster(weekly) 
qui eststo:reghdfe `y'_led `y' noq $control soe (`a'=l.soe_olym1) , abs(id week) cluster(weekly) 
}
}
...
esttab using summer.csv , $option   replace
 ...
 reghdfe amihud_led amihud c.co_rank##olympic  $full, abs(id week) cluster(weekly)
 reghdfe amihud_led amihud c.co_rank##summer  $full, abs(id week) cluster(weekly)
...
//outreg using manual,$outregopt replace
 reghdfe beta_led beta c.co_rank##soe_olym1 $full, abs(id week) cluster(weekly)
 reghdfe amihud_led amihud c.co_rank##soe_olym $full, abs(id week) cluster(weekly)
 reghdfe beta_led beta c.co_rank##soe_fifa $full, abs(id week) cluster(weekly)
...
esttab using triple.csv , $option   replace
 ...
 reghdfe beta_led beta c.co_rank##soe##olympic $full, abs(id week) cluster(weekly)
 reghdfe amihud_led amihud c.co_p_r##soe##olympic $full, abs(id week) cluster(weekly)
...
eststo clear
foreach a of varlist co_rank co_p_r lnco lnuser_p  {
qui eststo: reghdfe `a' soe_olym1 soe rep1-rep4 l.(amihud noq   $control ), abs(id week) cluster(weekly) 
}
esttab using pilot.csv , $optionfull   append
foreach a of varlist co_rank co_p_r lnco lnuser_p  {
qui eststo: reghdfe `a' soe_fifa soe rep1-rep4 l.(amihud noq   $control ) , abs(id week) cluster(weekly) 
}
esttab using pilot.csv , $optionfull   append
...
gl beta_eqn lntot_ask amihud co_p_r soe_olym1 soe $control //, abs(id week) cluster(weekly)
...
************************mannual heckman
eststo clear
qui eststo:heckman $beta_eqn, select($seleqn)  twostep //first vce(cluster weekly)

*-----------------------------------Prob of questions compare with reg non selection
eststo clear
qui eststo:reghdfe $beta_eqn, abs(id week) cluster(weekly)
...
preserve
collapse (mean) avg=`var' (median) med=`var', by( weekly)
tsset weekly
...
preserve
collapse (mean) beta=beta syn=syn amihud=amihud if year>2009, by( weekly)
tsset weekly
...
preserve
collapse (mean) tot_vol=tot_vol tot_vol_mv=tot_vol_mv idrisk_std=idrisk_std if year>2009, by( weekly)
tsset weekly
