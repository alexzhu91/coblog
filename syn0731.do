...
drop if bm<0 & size<0 & turnover<0
g lnbm=ln(bm+1)
...
g lnco=ln(co_p+1)
g co_attention=ln(link_stk/num_user+1) if num_user ~= 0
replace co_attention=0 if co_attention==.
sum co_attention
g lncmt_p=ln(num_cmt+1)
...
bys mdate: cumul(num_cmt), gen(cmt_rank) equal
bys mdate: cumul(TotInsHoldper), gen(inst_rank) equal
g inst_hld=inst_rank>.5
ta inst_hld year
sort id mdate
...
label var co_attention "log(1+co)"
...
gl option p not obs b(a2) label drop($control ) stats(r2 F N, labels(R2 F "No. of obs")) legend varlabels(_cons Constant) star(* 0.10 ** 0.05 *** 0.01)  
...
gl core lndist_stk dist_stk_detrend co_attention link_stk link_sum num_user dist_stk_p diverse_avg dist_stk lndiverse  lnco co_demean  co_p_r   co2_p_r co3_p_r cmt_rank co_rank 
...
*---------------Baseline with beta, dif funtion form, link_stk link_sum num_user dist_stk_p diverse_avg is already log & detrended-----------------------------------
*link avg is raw value, detrend post is really showing time pattern
cor lnnum_user lndist_stk  dist_stk_detrend co_attention co_p_r
sum lndist_stk dist_stk_detrend

eststo clear
loc core lndist_stk co_attention co_p_r //link_stk link_sum num_user dist_stk_p diverse_avg lndiverse link_avg lnco     co2_p_r co3_p_r cmt_rank co_rank 
loc beta beta syn pricedelay turnover  amihud //tot_vol idrisk_std skew 
foreach a of varlist `beta' {
foreach b of varlist `core' {
*qui eststo: reghdfe f.`a'  f.`b' $firmClu
qui eststo: reghdfe f.`a'  f.`b' `a' $firmClu
qui eststo: reghdfe f.`a'  f.`b' `a' f.lnnum_user $firmClu
*qui eststo: reghdfe f.`a' `b' $full
*qui eststo: reghdfe f.`a'  `b' `a' $full
}
}
esttab using final_base.csv , $option   title("Baseline with firmClu constraint") replace  //append
*---------------subsample,need to cut inst as ctrl var
gl control  lnsize lnbm  ret lret_6  coverage 
gl firmClu f.withq $control , abs(ym) cluster(id) 
...
eststo clear
loc core lndist_stk  co_attention co_p_r //dist_stk_detrend
loc beta beta syn pricedelay turnover amihud // tot_vol idrisk_std skew 
preserve
keep if inst_hld 
foreach a of varlist `beta' {
...
esttab using final_base.csv , $option   title("Subsample of inst_hld with inst ctrl not droped") append  
*---------------IV


*---------------robustness 1. cluster 2. max 3. detrend will get (-) on syn
eststo clear
loc core lndist_stk co_attention co_p_r 
loc beta beta syn pricedelay turnover  amihud //tot_vol idrisk_std skew 
foreach a of varlist `beta' {
foreach b of varlist `core' {
qui eststo: reghdfe f.`a'  f.`b' `a' f.lnnum_user $full
}
}
esttab using robust.csv , $option   title("Full FE and time cluster") append  //replace
eststo clear
loc core dist_stk_detrend //co_attention co_p_r 
loc beta beta syn pricedelay turnover  amihud //tot_vol idrisk_std skew 
foreach a of varlist `beta' {
foreach b of varlist `core' {
qui eststo: reghdfe f.`a'  f.`b' `a' $firmClu
qui eststo: reghdfe f.`a'  f.`b' `a' f.num_user_detrend $firmClu
}
}
esttab using robust.csv , $option   title("Detrend with firm cluster & num_user_detrend updated") append  //replace
eststo clear
preserve
keep if max 
loc core lndist_stk co_attention co_p_r 
...
esttab using robust.csv , $option   title("Full FE and time cluster & max") append  //replace
*---------------understand trend var opposite, two core must be detrend at the same time
reghdfe f.syn syn f.dist_stk_detrend f.num_user_detrend $firmClu
*---------------what determine detrend var: new info?
reghdfe f.lndist_stk f.lnnum_user $firmClu
reghdfe f.dist_stk_detrend f.rep4 f.lnnum_user f.TotInsHoldper  $firmClu
...
reghdfe f.syn syn f.lndist_stk f.lnnum_user $firmClu
gl time summer rep3 rep4 jan aug i.year //w/o i.year is same

reghdfe f.syn syn  f.lnnum_user  f.withq $control f.($time) (f.dist_stk_detrend=f.olympic), abs(id) 
reghdfe f.syn syn  f.lnnum_user  f.withq $control f.($time) (f.lndist_stk=f.olympic), abs(id) ffirst stage(first) 
xtivreg f.syn syn  f.lnnum_user  f.withq $control f.($time) (f.lndist_stk=f.fifa), fe first
xtivreg f.syn syn  f.lnnum_user  f.withq $control f.($time) (f.lndist_stk=f.rep2), fe first

loc beta   pricedelay
foreach a of varlist `beta' {
xtivreg f.`a'  `a'   f.lnnum_user  f.withq $control f.($time) (f.dist_stk_detrend=f.olympic), fe first
xtivreg f.`a'  `a'   f.lnnum_user  f.withq $control f.($time) (f.lndist_stk=f.olympic), fe first
}
reghdfe f.pricedelay pricedelay  f.lnnum_user  f.withq $control f.($time) (f.dist_stk_detrend=f.olympic), abs(id) 

reghdfe f.turnover turnover f.lndist_stk f.lnnum_user $firmClu
reghdfe f.turnover turnover f.lndist_stk f.lnnum_user $firmClu
reghdfe f.turnover turnover f.lndist_stk  $firmClu

reghdfe f.turnover turnover f.dist_stk_detrend f.num_user_detrend  $firmClu
g vol=ln(1+tot_vol_mv)
reghdfe f.vol vol f.dist_stk_detrend f.num_user_detrend  $firmClu
reghdfe f.vol vol f.lndist_stk f.lnnum_user  $firmClu
*dispersion, leverage, 
...
keep if withq
reghdfe f.syn syn f.lndist_stk f.lnnum_user $firmClu
reghdfe f.beta beta f.lndist_stk f.lnnum_user $firmClu

reghdfe f.syn syn f.co_p_r $firmClu
reghdfe f.beta beta f.co_p_r $full
eststo clear
loc core link_stk link_sum num_user dist_stk_p diverse_avg 
