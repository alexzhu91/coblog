...
*discuss one stk only count one time per week;
...
/*timeseries pattern of investors attention
movsum don't equal the real past year, since the week can be across many years
for simplicity, use past calendar year to denominate*/
proc sort data=user_stk nodupkey; by  &user &time; run;
proc expand data=user_stk  out=user_stk1 method = none; 
  by &user;
  id &time; 
	convert dist_stk = stk_max  / transformout=(MOVMax  5 trimleft 4);*get the previous month attention;
	convert dist_stk = stk_total  / transformout=(MOVsum  52  );*trimleft 51;
...
var stk_max stk_total;
where stk_total ne .;
run;
proc means  data=user_stk_yr n mean std min max;
var dist_stk_yr;
run;
*theory shld use distinct stk per year;
proc sql;
create table user_stk_final as
select *
from  user_stk a left join user_stk_yr b on a.&user=b.&user and year(a.&time)=b.year+1
order by &time, &user ;
...
select &time, stkcd,sum(dist_stk/dist_stk_yr) as diverse, sum(co) as co_&data, sum(co2) as co2_&data,
...
if &time ne .;
run;
proc sort data=stk1_p5w;by stkcd week;
*stk_p5w has all positive discussion;
proc sort data=rep;by stkcd week;
data week_co;
merge stk1_p5w weekrtn(in=c) rep;
by stkcd week;if c;
nodiverse=(diverse=.);
if year(week)>2009;
	array CO dist_stk: diverse co: num_user: num_cmt: ;
	do over CO;
	if CO=. then CO=0;
	end;
...
by week;
 model exret_led = diverse nodiverse lnsize turnover bm;
run;
