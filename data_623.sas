...
on =b.year & month(a.ym)=b.month & a.stkcd=b.stkcd
where a.stkcd ne "" 
order by stkcd, ym ;
...
by stkcd ym;
pricedelay=1-res/unr;keep stkcd ym delay;
...
label RET    = "Stock Return of Last Month ";
...
stk_x(rename=(co_num=co_x num_blogger=blogger_x) in=b) blogfull rep1-rep4 amihud_AVG(drop=_type_ in=c) delay_day;
by stkcd ym;if c;
...
if year>2009 then do;
	...
	if num_cmt_p5w ne 0 then co_p_r=co_p/num_user_p5w;
	else co_p_r=0;
	if num_cmt_xq ne 0 then co_x_r=co_x/num_user_xq;
	else co_p_x=0;
end;
...
*volume is in 1000s;
run;
proc sort nodupkey; by stkcd ym; run;
...
proc means data=stk_1st min  ;
var year(date_1st) ;
run;(format=date_1st date9.)
run;
